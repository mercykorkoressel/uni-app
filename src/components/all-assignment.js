...
export default function AllAssignment() {
  ...
  const fetchAssignment = async () => {
    try {
      const response = await axios.get('http://localhost:9000//assign');
      // Filter promotions to include only those created by the logged-in user
      const userAssignment = response.data.filter(assignment =>assignment.userId === userInfo.id);
...
<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
  ...
  <main class="p-6 sm:p-10 space-y-6">
    ...
    <section class="grid md:grid-cols-2 xl:grid-cols-2 xl:grid-rows-2 xl:grid-flow-col gap-6">
     ...
     <h1 class="font-semibold text-4xl mb-4">Assignments</h1>
    
 </div>
 <div class="flex flex-wrap gap-4 p-6 justify-center text-lg font-serif">
 {assignment.map(assignment => (
    <a href="#"
    key={assignment._id}  class="bg-gray-100 flex-grow text-black border-l-8 border-purple-500 rounded-md px-3 py-2 w-full md:w-5/12 lg:w-3/12">{assignment.course}
